...
when_to_use: Triggered by requests like "fix my test coverage", "write tests for uncovered lines", "improve coverage on my changes", "add missing tests for the diff", or "get my coverage to 80%".
allowed-tools: Bash(node *), Bash(npx *), Read, Edit, Write
---
...
The `diff-coverage` skill must also be installed — its `run.cjs` script is reused here.

## Invocation

```bash
node "${CLAUDE_SKILL_DIR}/../diff-coverage/scripts/run.cjs" <command> [options]
```
...
Run `measure` to get the baseline report:

```bash
node "${CLAUDE_SKILL_DIR}/../diff-coverage/scripts/run.cjs" measure \
  --cwd <project-dir> \
  --base <git-ref>
```
...
- Follow the naming convention: `describe` names the unit under test, `it`/`test` describes the behaviour
- Use `it.each` for multiple similar inputs/outputs
- Test error paths (thrown exceptions) using `expect(() => fn()).toThrow(...)` or `await expect(promise).rejects.toThrow(...)`
- Do not import or call any source file other than the one being tested
- Do not modify source files

### Step 4 — Verify tests pass

Run the test suite scoped to the files you modified. If the project uses Jest:
...
Run `measure` again with the same arguments as Step 1:
...
Compare the new per-file percentages to the baseline from Step 1.

### Step 6 — Iterate or conclude

- If uncovered lines remain and coverage improved since the last iteration, return to Step 2 for the still-uncovered files.
...
- If all changed files reach 100% line coverage, or if a `--threshold` was specified and all files now meet it, report success.
